...
int main(int argc, char ** argv) {
    ...
    // Testing that two clients can speak to each other using SAYCONT REVC,
    // and that the terminator is correctly confirmed
    char buffer[2048] = {0};
    char * identifier1 = "strawberry"; // C1: strawberry
    char * domain1 = "test_folder";
    char * rd_pipe1 = "test_folder/strawberry_RD";
    char * wr_pipe1 = "test_folder/strawberry_WR";
    char * identifier2 = "mango"; // C2: mango
    char * domain2 = "test_folder";
    char * rd_pipe2 = "test_folder/mango_RD";
    char * wr_pipe2 = "test_folder/mango_WR";
    ...
    char * message;
    // client 1 send a SAYCONT with term 255, client 2 should receive a RECVCONT
    // with the same terminator
    char temp[2048] = {0};
    message = "Good morning, this is the first part of the message.";
    memset(buffer, 0, 2048);
    buffer[0] = 0x1;
    strcpy(temp, message);
    memory_splice(buffer+2, temp, 1790);
    buffer[2048-1] = '0'+207; // 255
    ...
    if (buffer[2048-1] != '0'+207) {
        return 1; // Termination is different
    }
